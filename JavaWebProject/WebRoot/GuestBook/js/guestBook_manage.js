				...
				$.ajax({
					...
					success : function (guestBook, response, status) {
						...
						if (guestBook) { 
							...
							$("#guestBook_userObj_userInfoname_edit").combobox({
								url:"UserInfo/listAll",
							    valueField:"userInfoname",
							    textField:"name",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#guestBook_userObj_userInfoname_edit").combobox("select", guestBook.userObjPri);
									//var data = $("#guestBook_userObj_userInfoname_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#guestBook_userObj_userInfoname_edit").combobox("select", data[0].userInfoname);
						            //}
								}
							});
