...
function initUserInfoManageTool() {
	userInfo_manage_tool = {
		...
		search: function() {
			...
			queryParams["userInfoname"] = $("#userInfoname").val();
				...
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						for (var i = 0; i < rows.length; i ++) {
							userInfonames.push(rows[i].userInfoname);
						}
						$.ajax({
							type : "POST",
							url : "UserInfo/deletes",
							data : {
								userInfonames : userInfonames.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "UserInfo/" + rows[0].userInfoname +  "/update",
					type : "get",
					data : {
						//userInfoname : rows[0].userInfoname,
					},
					beforeSend : function () {
						$.messager.progress({
							text : "正在获取中...",
