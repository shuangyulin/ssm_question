...
<%
    ...
    String userInfoname = (String)request.getAttribute("userInfoname"); //用户名查询关键字
	...
	<div class="col-md-9 wow fadeInLeft">
		...
		<div class="row">
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>UserInfo/<%=userInfo.getUserInfoname() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=userInfo.getUserPhoto()%>" /></a>
			     <div class="showFields">
			     	<div class="field">
	            		用户名:<%=userInfo.getUserInfoname() %>
			     	</div>
			     	<div class="field">
			     	...
			     	<div class="field">
	            		邮箱地址:<%=userInfo.getEmail() %>
			     	</div>
			        <a class="btn btn-primary top5" href="<%=basePath %>UserInfo/<%=userInfo.getUserInfoname() %>/frontshow">详情</a>
			        <a class="btn btn-primary top5" onclick="userInfoEdit('<%=userInfo.getUserInfoname() %>');" style="display:none;">修改</a>
			        <a class="btn btn-primary top5" onclick="userInfoDelete('<%=userInfo.getUserInfoname() %>');" style="display:none;">删除</a>
			     </div>
			</div>
			<%  } %>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="userInfoQueryForm" id="userInfoQueryForm" action="<%=basePath %>UserInfo/frontlist" class="mar_t15" method="post">
			<div class="form-group">
				<label for="userInfoname">用户名:</label>
				<input type="text" id="userInfoname" name="userInfoname" value="<%=userInfoname %>" class="form-control" placeholder="请输入用户名">
			</div>
			<div class="form-group">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="userInfo_userInfoname_edit" class="col-md-3 text-right">用户名:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="userInfo_userInfoname_edit" name="userInfo.userInfoname" class="form-control" placeholder="请输入用户名" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="userInfo_email_edit" class="col-md-3 text-right">邮箱地址:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="userInfo_email_edit" name="userInfo.email" class="form-control" placeholder="请输入邮箱地址">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="userInfo_address_edit" class="col-md-3 text-right">用户住址:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="userInfo_address_edit" name="userInfo.address" class="form-control" placeholder="请输入用户住址">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="userInfo_userPhoto_edit" class="col-md-3 text-right">个人照片:</label>
		  	 <div class="col-md-9">
...
/*弹出修改用户信息界面并初始化数据*/
function userInfoEdit(userInfoname) {
	$.ajax({
		url :  basePath + "UserInfo/" + userInfoname + "/update",
		type : "get",
		dataType: "json",
...
/*删除用户信息信息*/
function userInfoDelete(userInfoname) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "UserInfo/deletes",
			data : {
				userInfonames : userInfoname,
			},
...
function ajaxUserInfoModify() {
	$.ajax({
		url :  basePath + "UserInfo/" + $("#userInfo_userInfoname_edit").val() + "/update",
		type : "post",
		dataType: "json",
